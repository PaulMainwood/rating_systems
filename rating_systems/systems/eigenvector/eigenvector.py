"""
Eigenvector centrality (B-Score) rating system.

Builds a directed, time-decayed network from match results: an edge from
player i to player j (with weight) exists when i loses to j.  Power
iteration on the transposed adjacency matrix (with a small uniform
perturbation for Perron-Frobenius irreducibility) gives each player a
centrality score that reflects not just how many opponents they beat,
but the quality of those opponents recursively.

Uses array-based edge storage with dict index for O(1) lookup and
Numba-accelerated bulk decay.

Reference: Arcagni, Candila & Grassi, "A New Model for Predicting the
Winner in Tennis Based on the Eigenvector Centrality", Annals of Operations
Research 325, 615-632 (2023).
"""
...
from ._numba_core import predict_proba_batch, predict_single, decay_edges_inplace
...
class EigenvectorConfig:
    """Configuration for Eigenvector Centrality rating system."""

    half_life: float = 365.0      # Decay half-life in days
    scale: float = 200.0          # Logistic scale
    centrality_scale: float = 10000.0  # Scale centrality to usable range
    recompute_interval: int = 7   # Recompute eigenvector every N days
    min_edges: int = 10           # Min non-zero edges before computing centrality
    damping: float = 1e-6         # Perturbation for Perron-Frobenius guarantee


class EigenvectorCentrality(RatingSystem):
    """
    Eigenvector centrality (B-Score) rating system.

    Builds a directed network from match results and computes centrality
    via power iteration with a small damping perturbation.  The dominant
    eigenvector of (A^T + damping * 11^T) gives each player a score
    that is guaranteed to be unique and non-negative (Perron-Frobenius).

    Edge storage uses parallel numpy arrays with a Python dict index for
    O(1) lookup during per-game updates.  Bulk decay during matrix
    construction is Numba-accelerated.

    Parameters:
        half_life: Exponential decay half-life in days (default: 365)
        scale: Logistic scale for predictions (default: 200)
        centrality_scale: Multiplier to map centrality [0,1] to Elo-like range
    ...
    def __init__(
        ...
        self._edge_index: Dict[tuple, int] = {}  # (loser, winner) -> array idx
        self._n_edges = 0
    ...
    def _ensure_capacity(self, needed: int) -> None:
        """Grow edge arrays if capacity is insufficient."""
    ...
    def _init_edge_arrays(self, capacity: int) -> None:
        """Allocate fresh edge arrays with the given capacity."""
    ...
    def _rebuild_edge_index(self) -> None:
        """Rebuild the (src, dst) -> index dict from the edge arrays."""
        self._edge_index = {}
        src = self._edge_src
        dst = self._edge_dst
    ...
    def _update_adjacency(self, batch: GameBatch) -> None:
        """Update edge arrays with a batch of unweighted games."""
        day = batch.day
        dr = self._decay_rate
    ...
    def _build_sparse_matrix(self, day: int) -> csr_matrix:
        """Build CSR matrix from edge arrays, decaying weights to ``day``.

        Modifies edge_weight and edge_day in-place (all timestamps set to
        ``day`` after this call).
        """
        n = self._n_edges
        if n == 0:
        ...
        # Bulk decay via Numba
        decay_edges_inplace(
            self._edge_weight, self._edge_day, n, day, self._decay_rate,
        )

        # Vectorised filter: keep edges above prune threshold
    ...
    def _recompute_centrality(self, day: int) -> None:
        ...
        Uses scipy's implicitly-restarted Arnoldi iteration (ARPACK) on
        a damped operator (A^T + eps * 11^T).  The damping makes the
        operator irreducible, guaranteeing a unique, non-negative
        eigenvector.  Warm-starts from the previous centrality.
        """
        ...
        # Damped operator: v -> A^T @ v + eps * sum(v)
        def matvec(v):
    ...
    def _update_ratings(self, batch: GameBatch, ratings: PlayerRatings) -> None:
        """Update adjacency matrix and periodically recompute centrality."""
        if len(batch) == 0:
            return

        self._update_adjacency(batch)
    ...
    def fit(
        ...
        """Fit on a dataset by building the adjacency matrix day-by-day.

        Skips intermediate eigenvector recomputes during initial fit
        (only edge updates matter).  Computes the eigenvector once at
        the end.
        """
        ...
        # Build edges only (skip expensive eigenvector recomputes)
    ...
    def save_state(self, path: str) -> None:
        ...
        metadata = {
            "system_class": "EigenvectorCentrality",
    ...
    def load_state(self, path: str) -> None:
        ...
        # Support both new (edge_src/dst/weight/day) and legacy (edge_keys/vals)
        ...
        if src is not None:
            ...
            # Legacy: edge_keys (N,2) int32, edge_vals (N,2) float64
    ...
    def reset(self) -> "EigenvectorCentrality":
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"EigenvectorCentrality(half_life={self.config.half_life}, "
