...
@njit(cache=True, fastmath=True)
def update_single_player_jacobi(
    player_id: int,
    player_offsets: np.ndarray,
    pd_days: np.ndarray,
    pd_r: np.ndarray,
    ...
    """
    Update a single player's ratings using Newton-Raphson (Jacobi variant).
...
def run_iteration_jacobi(
    ...
    """
    Run one Jacobi Newton-Raphson iteration for all players in parallel.
    ...
    for player_id in prange(num_players):
        change = update_single_player_jacobi(
            player_id,
            player_offsets,
            pd_days,
            pd_r,
...
@njit(cache=True, fastmath=True, parallel=True)
def run_iteration_jacobi_active(
    num_players: int,
    active: np.ndarray,
    player_offsets: np.ndarray,
    pd_days: np.ndarray,
    ...
    """
    Run one Jacobi Newton-Raphson iteration, only updating active players.
...
def run_all_iterations_accelerated(
    ...
    """
    Run Newton-Raphson with optional active-set, Anderson acceleration, and
    Jacobi parallelism.
    ...
    for iteration in range(max_iterations):
        ...
        if use_jacobi:
            if use_active_set:
                max_change = run_iteration_jacobi_active(
                    num_players, active, player_offsets, pd_days, pd_r,
                    pd_r_read, pd_game_offsets, pd_game_opp_pd,
                    pd_game_score, w2_r, convergence_threshold,
                    pd_to_player,
                )
            else:
                max_change = run_iteration_jacobi(
